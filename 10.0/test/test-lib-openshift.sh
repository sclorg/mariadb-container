...
function ct_os_deploy_cmd_image() {
  local image_name=${1}
  oc get pod command-app && echo "command POD already running" && return 0
...
EOF
  ...
  while [ $SECONDS -lt 60 ] ; do
    ct_os_cmd_image_run "echo 24" && echo "DONE" && return 0
    sleep 3
    echo -n "."
  done
  echo "FAIL"
