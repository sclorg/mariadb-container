...
```
$ podman run -d --name mariadb_database -e MYSQL_USER=user -e MYSQL_PASSWORD=pass -e MYSQL_DATABASE=db -p 3306:3306 rhel8/mariadb-103
```
...
```
$ oc new-app mariadb:10.3~./image-configuration/ --name my-mariadb-rhel7
```

or via s2i:

```
$ s2i build ./image-configuration/ rhscl/mariadb-103-rhel7 my-mariadb-rhel7
```
